...
function StatCard({ stat, index }: { stat: (typeof stats)[0]; index: number }) {
  ...
  return (
    ...
    >
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isOrange ? "bg-[#fffaf0] border border-[#f9d59f]" : "bg-[#f2f5fc] border border-[#dce4f7]"}`}>
        <Icon className={`w-6 h-6 ${isOrange ? "text-[#fd9f09]" : "text-[#031033]"}`} strokeWidth={1.8} />
      </div>
      <div>
        <div className={`text-4xl font-extrabold mb-1 ${isOrange ? "text-[#fd9f09]" : "text-[#031033]"}`}>{stat.value}</div>
...
export default function StatsSection() {
  return (
    <section id="stats" className="section-pad section-white relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#dce4f7] to-transparent pointer-events-none" />
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#f9d59f] to-transparent pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#031033] mb-4">
            Built for{" "}
            <span className="text-[#fd9f09]">Speed, Security &amp; Growth</span>
          </h2>
        </div>
